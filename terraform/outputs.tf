...
# ── Debug/"console.log" style outputs ────────────────────────────────────────

output "debug_region" {
  description = "Primary region (handy for quick checks / CI logs)."
  value       = var.primary_region
}

output "debug_primary_vpc_id" {
  description = "Primary VPC id (handy for quick checks / CI logs)."
  value       = aws_vpc.primary.id
}

output "debug_alb_dns" {
  description = "ALB DNS name if you provide it via primary_endpoint (this template doesn't create an ALB by default)."
  value       = var.primary_endpoint
}

output "debug_cloudfront_domain" {
  description = "CloudFront domain if you provide it via secondary_endpoint (this template doesn't create CloudFront by default)."
  value       = var.secondary_endpoint
}
